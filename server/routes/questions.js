...
module.exports = {
	...
	getAll: (req, res) => {
		Question.where('deleted').ne('true').select('id description category answer type time dateModified').exec((err, questions) => {
			if (err) {
				sendErr(res, err);
			} else {
				sendSuccess(res, {
	...
	getOne: (req, res) => {
		Question.where('_id').equals(req.params.id).select('id description category answer type time dateModified').exec((err, question) => {
			if (err) {
				sendErr(res, err);
			} else {
				sendSuccess(res, {
